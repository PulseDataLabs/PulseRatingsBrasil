...
def get_moodys_session(logger, base_url: str = BASE_URL):
    ...
    for profile in IMPERSONATE_PROFILES:
        try:
            ...
            if resp.status_code == 200:
                logger.info(f"Conexão direta bem-sucedida usando perfil {profile}! Nenhum proxy será utilizado.")
...
def ensure_moodys_file_downloaded(logger) -> Path:
    ...
    if CACHE_FILE.exists():
        ...
        if file_size > 1024 * 1024 and file_age < CACHE_MAX_AGE_SECONDS:
            logger.info(f"Reutilizando arquivo Excel em cache: {CACHE_FILE} ({file_size / 1024 / 1024:.2f} MB, {file_age / 60:.1f} min)")
            return CACHE_FILE
    ...
    for attempt in range(1, 4):
        try:
            ...
            if len(content) < 1024 * 1024:
                raise ValueError(f"Arquivo baixado muito pequeno ({len(content)} bytes). Possível erro na resposta.")

            with temp_cache.open("wb") as f:
                f.write(content)

            temp_cache.replace(CACHE_FILE)
            logger.info(f"Download concluído com sucesso: {len(content) / 1024 / 1024:.2f} MB salvos em {CACHE_FILE}")
            return CACHE_FILE
        except Exception as e:
    ...
    with zf.open("xl/worksheets/sheet1.xml") as f:
        for event, elem in ET.iterparse(f, events=("end",)):
            if elem.tag.endswith("}row"):
                ...
                if file_date is None and "G" in cell_dict and "Data de Atualização" in str(cell_dict.get("G", "")):
