...
def inferir_tipo_oracle(col_name: str, series: pd.Series) -> str:
    ...
    candidatos_data = ["data", "date", "dt_", "_dt", "periodo", "anomes", "ano_mes", "data_base", "data_referencia", "data_captura"]
...
def persistir_no_oracle(
    ...
    if not table_exists:
        ...
        columns_ddl = [f"{san_col} {inferred_types[orig_col]}" for san_col, orig_col in zip(sanitised_columns, cabecalho)]
    ...
    colunas_data_candidatas = ["anomes", "ano_mes", "data_base", "data_referencia", "data_captura", "data", "dt_captura", "dt_referencia", "periodo"]
    date_col = None
    for col in cabecalho:
    ...
    if date_col:
        ...
        logger.info(f"Tabela {table_name} possui coluna de data ({date_col}). Executando deleção de {len(unique_periods)} períodos específicos antes da inserção incremental.")
        ...
        with engine.begin() as conn:
            for period in unique_periods:
                ...
                if cleaned_period is None:
                    conn.execute(
                        sqlalchemy.text(f"DELETE FROM {table_name} WHERE {san_date_col} IS NULL")
                    )
                else:
                    conn.execute(
        ...
        logger.info(f"Tabela {table_name} não possui colunas de data/período. Executando TRUNCATE TABLE e carga completa.")
...
def executar_carga(
    ...
    insert_sql = f"INSERT INTO {table_name} ({', '.join(sanitised_columns)}) VALUES ({', '.join(placeholders)})"
